...
//  Copyright © 2017年 VEUISDK. All rights reserved.
//
...
@interface UIMusicInfo : NSObject
...
/**视频输出方式
 */
typedef NS_ENUM(NSInteger, VEPROPORTIONTYPE){
    VEPROPORTIONTYPE_AUTO       = 0,//自动
    VEPROPORTIONTYPE_LANDSCAPE  = 1,//横
    VEPROPORTIONTYPE_SQUARE     = 2//正方形
};
...
/**截取之后返回的类型
 */
typedef NS_ENUM(NSInteger, VECutVideoReturnType){
    VECutVideoReturnTypePath,   //真实截取
    VECutVideoReturnTypeTime,   //返回时间段
    VECutVideoReturnTypeAuto    //动态截取
...
/**水印位置
 */
typedef NS_ENUM(NSInteger, VEDouYinRecordType){
    VEDouYinRecordType_Story, //随拍
    VEDouYinRecordType_Record,  //录制
};

typedef NS_ENUM(NSInteger, VESingleFunctionType){
    VESingleFunctionType_Transcoding    = 0,    //转码
    VESingleFunctionType_Reverse,               //倒放
    VESingleFunctionType_Crop,                  //裁剪
    VESingleFunctionType_Intercept,             //截取
    VESingleFunctionType_Compress,              //压缩
    VESingleFunctionType_Transition,            //转场
    VESingleFunctionType_Adjust,                //调色
    VESingleFunctionType_Speed,                 //变速
    VESingleFunctionType_VoiceFX,               //变声
    VESingleFunctionType_Dubbing,               //配音
    VESingleFunctionType_ClipEditing,           //片段编辑
    VESingleFunctionType_Cover,                 //封面
    VESingleFunctionType_Caption,               //字幕
    VESingleFunctionType_Sticker,               //贴纸
    VESingleFunctionType_Filter,                //滤镜
    VESingleFunctionType_Effect,                //特效
    VESingleFunctionType_Dewatermark,           //去水印
    VESingleFunctionType_Collage,               //画中画
    VESingleFunctionType_Doodle,                //涂鸦
...
/** 默认视频输出方式（自动，横屏，1 ：1）
 */
@property (nonatomic,assign) VEPROPORTIONTYPE  proportionType;
/** 倒放 (default true)
 */
...
// faceU美颜设置
@interface FaceUBeautyParams : NSObject<NSCopying>
...
@property (nonatomic, strong) FaceUBeautyParams           *faceUBeautyParams;
...
/*传入需要录制时播放的音乐
 */
@property (nonatomic, strong) UIMusicInfo          * _Nullable musicInfo;
...
typedef void(^VECallbackBlock) (NSString * videoPath);//编辑完成导出结束回调 //EditCompletionBloc
typedef void(^VERecordCallbackBlock) (int result,NSString *path,UIMusicInfo *music);

typedef void(^VEAddVideosAndImagesCallbackBlock) (NSMutableArray<NSURL*> * list);
/**相册选择完成返回一个URL数组
 */
typedef void(^OnAlbumCallbackBlock) (NSMutableArray <NSURL*> * urls);
typedef void(^VEAddVideosCallbackBlock) (NSMutableArray<NSURL*> * list);
typedef void(^VEAddImagesCallbackBlock) (NSMutableArray<NSURL*> * list);

//编辑取消回调
typedef void(^VECancelBlock) (void);   //EditCancelBlock
//编辑失败回调
typedef void(^VEFailBlock) (NSError * error);
...
@protocol VEUISDKDelegate <NSObject>
...
/** 设置faceU美颜参数
 */
- (void)faceUBeautyParamChanged:(FaceUBeautyParams *)beautyParams;
